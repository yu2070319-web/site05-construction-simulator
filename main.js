...
import {CraneController} from './systems/CraneController.js';
import {InputManager} from './systems/InputManager.js';
import {ExcavatorController} from './systems/ExcavatorController.js';
...
cyl(p,M.darkSteel,0,.55,0,.55,.25);const upper=group(p);box(upper,M.yellow,0,.78,0,1.7,.4,1.25);box(upper,M.yellow,-.55,1.17,-.22,.8,.72,.69);box(upper,M.glass,-.55,1.24,.135,.58,.5,.025);box(upper,M.glass,-.12,1.24,-.22,.025,.5,.56);box(upper,M.yellow,-.55,1.57,-.22,.92,.1,.82);box(upper,M.black,-.72,.99,.4,.43,.04,.25);
const boom=group(upper,.28,.9,-.2);beam(boom,M.yellow,[0,0,0],[1.25,0,0],.24);beam(boom,M.steel,[.05,.15,.15],[1.05,.15,.15],.07);
...
boom.rotation.z=.82;stick.rotation.z=-1.42;bucket.rotation.z=-.45;const rig={id:'excavator',root:p,upper,boom,stick,bucket,limits:{boom:[.18,1.18],stick:[-1.85,-.45],bucket:[-1.2,.45]}};p.userData.rig=rig;return p;}
...
const soilChunks=[];for(let i=0;i<28;i++){const a=i*.91,r=.25+(i%6)*.16,c=part(scene,new THREE.DodecahedronGeometry(.18+(i%3)*.025,0),M.soil,9.45+Math.cos(a)*r,.68+(i%4)*.1,-3.1+Math.sin(a)*r,1,1,1);c.rotation.set(a*.3,a,.2);soilChunks.push(c);}
...
function createCraneLoads(){const loads=[{mesh:rebarCargo,type:'rebar',label:'钢筋束',half:{x:1.08,y:.2,z:.26},attachY:.18}];const wood=group(scene,-5.8,.84,-7.25);wood.userData={liftable:true,type:'wood',label:'木方托盘'};for(let y=0;y<3;y++)for(let z=-.42;z<=.42;z+=.28)box(wood,M.wood,0,y*.14,z,1.55,.11,.2);for(const x of [-.55,.55])beam(wood,M.steel,[x,.38,-.52],[x,.38,.52],.035);loads.push({mesh:wood,type:'wood',label:'木方托盘',half:{x:.8,y:.28,z:.58},attachY:.32});const precast=group(scene,-3.7,.95,-7.4);precast.userData={liftable:true,type:'precast',label:'预制混凝土梁'};box(precast,M.concrete,0,0,0,2.3,.42,.55);for(const x of [-.72,.72]){const eye=part(precast,new THREE.TorusGeometry(.11,.025,6,12),M.steel,x,.27,0,1,1,1);eye.rotation.x=Math.PI/2;}loads.push({mesh:precast,type:'precast',label:'预制混凝土梁',half:{x:1.15,y:.23,z:.28},attachY:.32});const equipment=group(scene,1.5,1.04,-7.55);equipment.userData={liftable:true,type:'large',label:'大型机电设备箱'};box(equipment,M.darkSteel,0,0,0,1.45,.85,1.05);for(const x of [-.55,.55])for(const z of [-.35,.35])cyl(equipment,M.steel,x,.5,z,.055,.2);sign(equipment,'6.0 t',0,.05,.54,.85,.25,'#293b35');loads.push({mesh:equipment,type:'large',label:'大型机电设备箱',half:{x:.74,y:.44,z:.54},attachY:.52});for(const load of loads)if(!liftables.includes(load.mesh))liftables.push(load.mesh);return loads;}
const craneLoads=createCraneLoads();
...
const craneInput=new InputManager();
const cranePhysics=new CraneController({rig:cranes[0],cargos:craneLoads,physics:physicsWorld,onStatus:message=>toast(message)});
const excavatorPhysics=new ExcavatorController({rig:excavator.userData.rig,physics:physicsWorld,soil:soilChunks,truck,onDig:load=>toast(`Bucket Load ${load}%`),onDump:load=>toast(`卡车载土 ${load}%`)});
...
const simulator=(()=>{
  ...
  const configs={
    ...
    excavator:{title:'挖掘机 01',type:'双履带 · ISO 双摇杆',buttons:[]},
  ...
  const excavInput={leftTrack:0,rightTrack:0,leftX:0,leftY:0,rightX:0,rightY:0},excavPointers=new Map();
  ...
  const resetExcavatorInput=()=>{Object.keys(excavInput).forEach(k=>excavInput[k]=0);excavPointers.clear();excavatorControls.querySelectorAll('.active').forEach(e=>e.classList.remove('active'));excavatorControls.querySelectorAll('.hydraulic-base i').forEach(e=>e.style.transform='translate3d(0,0,0)');};
  const stop=()=>{input.clear();pointerActions.clear();resetJoystick();resetExcavatorInput();grid.querySelectorAll('.active').forEach(b=>b.classList.remove('active'));};
  ...
  function enter(id=selectedId){if(!configs[id])return;stop();selectedId=id;mode=id;active=resolve(id);if(id==='craneA')cranePhysics.audio.unlock();if(id==='excavator')excavatorPhysics.audio.unlock();cameraMode='sandbox';viewAdjust=false;controls.enabled=false;controls.autoRotate=false;document.querySelector('#rotate').setAttribute('aria-pressed','false');document.body.classList.add('controlling');document.body.classList.toggle('excavator-mode',id==='excavator');document.body.classList.remove('view-adjust');excavatorControls.hidden=id!=='excavator';document.querySelector('#info').hidden=true;panel.hidden=false;document.querySelector('#control-title').textContent=configs[id].title;document.querySelector('#control-type').textContent=configs[id].type;renderButtons(configs[id]);cameraButton.hidden=false;cameraButton.textContent='📷';cameraButton.setAttribute('aria-label','切换相机：沙盘视角');adjustButton.textContent='调整视角';cargoButton.hidden=true;updateAction();snapCamera();if(innerWidth<innerHeight&&!sessionStorage.getItem('landscape-tip')){toast('横屏操作体验更好');sessionStorage.setItem('landscape-tip','1');}}
  function exit(){stop();mode='observe';active=null;viewAdjust=false;panel.hidden=true;excavatorControls.hidden=true;document.body.classList.remove('controlling','view-adjust','excavator-mode');controls.enabled=true;cameraMode='sandbox';resetCamera();}
  ...
  function updateTelemetry(){if(mode.startsWith('crane')){if(mode==='craneA'){const p=cranePhysics.snapshot();telemetry.innerHTML=`<div>LOAD<b>${p.loadKg?(p.loadKg/1000).toFixed(1)+' t':p.loadLabel||'空钩'}</b></div><div>CAPACITY<b>${(p.capacityKg/1000).toFixed(1)} t</b></div><div>RADIUS / HOOK<b>${active.trolleyX.toFixed(1)} / ${(active.height-active.cableLength).toFixed(1)} m</b></div><div>SWING<b>${p.swingDeg.toFixed(1)}°</b></div>`;}else{const d=worldDistanceXZ(active.block,rebarCargo);telemetry.innerHTML=`<div>HOOK<b>${(active.height-active.cableLength).toFixed(1)} m</b></div><div>RADIUS<b>${active.trolleyX.toFixed(1)} m</b></div><div>DISTANCE<b>${Math.hypot(d.flat,d.vertical).toFixed(1)} m</b></div>`;}}else if(mode==='excavator'){const p=excavatorPhysics.snapshot();telemetry.innerHTML=`<div>BUCKET LOAD<b>${p.bucketLoad}%</b></div><div>左履带<b>${p.leftTrack.toFixed(2)}</b></div><div>右履带<b>${p.rightTrack.toFixed(2)}</b></div><div>TRUCK LOAD<b>${p.truckLoad}%</b></div>`;}else{const speed=Math.abs(joystick.y)>.05?'行驶':'静止';telemetry.innerHTML=`<div>状态<b>${speed}</b></div><div>转向<b>${joystick.x<-.1?'左':joystick.x>.1?'右':'直行'}</b></div><div>${mode==='forklift'?'货叉':'视角'}<b>${mode==='forklift'?`${active.root.userData.forkHeight.toFixed(1)} m`:cameraMode==='cab'?'驾驶室':'外部'}</b></div>`;}}
  function updateAction(){nearCargo=null;if(mode.startsWith('crane')){if(mode==='craneA'){const state=cranePhysics.snapshot(),ready=cranePhysics.canConnect(),near=state.riggingDistance<1.2;cargoButton.hidden=!(cranePhysics.attached||near);cargoButton.disabled=!cranePhysics.attached&&!ready;cargoButton.textContent=cranePhysics.attached?'解除吊索':ready?'连接吊索':'接近吊点 '+state.riggingDistance.toFixed(2)+'m';const cap=cranePhysics.capacity();status.textContent=!cap.ok?'⚠ 超出允许载荷':cranePhysics.attached?'物理吊挂中':ready?'吊点已对准':near?'继续微调吊钩':'寻找货物吊点';updateTelemetry();return;}if(active.cargo){cargoButton.hidden=false;cargoButton.textContent='放下';status.textContent='运输中';updateTelemetry();return;}cargoButton.hidden=true;status.textContent='等待操作';}else if(mode==='forklift'){if(active.root.userData.load){cargoButton.hidden=false;cargoButton.textContent='卸货';status.textContent='已装载';}else{const d=worldDistanceXZ(active.root.userData.fork,palletCargo);nearCargo=palletCargo.parent===scene&&d.flat<1.8&&d.vertical<1.1?palletCargo:null;cargoButton.hidden=!nearCargo;cargoButton.textContent='装载';status.textContent=nearCargo?'可装载':'接近托盘';}}else{cargoButton.hidden=true;status.textContent='控制中';}updateTelemetry();}
  function cargoAction(){if(mode==='craneA'){if(cranePhysics.attached){const dropped=cranePhysics.activeLoad;cranePhysics.disconnect();toast(`${dropped.label} 已解除吊索`);checkTask(dropped.mesh);}else if(cranePhysics.connect()){const load=cranePhysics.activeLoad;toast(`${load.label} · ${(load.mass/1000).toFixed(1)} t 已连接`);}}else if(mode.startsWith('crane')){toast('物理吊装任务请使用塔吊 A');}else if(mode==='forklift'){const root=active.root;if(root.userData.load){const distance=root.position.distanceTo(root.userData.loadOrigin);scene.attach(root.userData.load);root.userData.load.position.y=Math.max(.86,root.userData.load.position.y);root.userData.load=null;if(task===1&&distance>4){toast('货物已卸下');completeTask();}else toast(task===1?'运输距离不足，请继续运往道路另一端':'货物已卸下');}else if(nearCargo){root.userData.fork.attach(nearCargo);nearCargo.position.set(-1.35,.28,0);root.userData.load=nearCargo;root.userData.loadOrigin=root.position.clone();toast('托盘已装载');}}updateAction();}
  ...
  function operateCrane(rig,dt){if(rig.id==='craneA'){cranePhysics.update(craneInput.read(joystick,input),dt);return;}rig.rotor.rotation.y+=joystick.x*dt*.72;rig.cableLength=THREE.MathUtils.clamp(rig.cableLength+((input.has('hookDown')?1:0)-(input.has('hookUp')?1:0))*dt*3.1,1.1,rig.height-1);rig.trolleyX=THREE.MathUtils.clamp(rig.trolleyX+((input.has('trolleyOut')?1:0)-(input.has('trolleyIn')?1:0))*dt*3,1.25,8.1);rig.trolley.position.x=rig.trolleyX;updateCraneCable(rig);}
  function operateExcavator(rig,dt){excavatorPhysics.update(excavInput,dt);}
  function equipmentCab(root){if(mode==='excavator')return active.upper.localToWorld(new THREE.Vector3(-.55,1.38,.1));return new THREE.Vector3(-.55,1.45,0).applyQuaternion(root.quaternion).add(root.position);}
  function equipmentLook(root){if(mode==='excavator')return active.upper.localToWorld(new THREE.Vector3(2,1.05,0));return new THREE.Vector3(2,1,0).applyQuaternion(root.quaternion).add(root.position);}
  function snapCamera(){if(mode==='observe'||!active)return;const root=active.root,deviceFocus=root.position.clone().add(new THREE.Vector3(0,mode.startsWith('crane')?active.height*.46:1,0)),focus=new THREE.Vector3();(mode.startsWith('crane')?active.block:root).getWorldPosition(focus);if(cameraMode==='sandbox')camera.position.copy(root.position).add(mode.startsWith('crane')?new THREE.Vector3(26,21,26):new THREE.Vector3(12,9,12));else if(cameraMode==='chase'){const offset=mode.startsWith('crane')?new THREE.Vector3(5,4,5):new THREE.Vector3(-7,5.2,0).applyQuaternion(root.quaternion);camera.position.copy(focus).add(offset);}else camera.position.copy(mode.startsWith('crane')?new THREE.Vector3(.3,active.height+.7,1.2).add(root.position):equipmentCab(root));camera.lookAt(cameraMode==='sandbox'?deviceFocus:cameraMode==='cab'?(mode.startsWith('crane')?focus:equipmentLook(root)):focus);}
  function followCamera(dt){if(mode==='observe'||viewAdjust||cameraMode==='sandbox')return;const root=active.root,focus=new THREE.Vector3();(mode.startsWith('crane')?active.block:root).getWorldPosition(focus);let desired,look;if(mode.startsWith('crane')){desired=cameraMode==='cab'?new THREE.Vector3(.3,active.height+.7,1.2).add(root.position):focus.clone().add(new THREE.Vector3(5,4,5));look=focus;}else{desired=cameraMode==='cab'?equipmentCab(root):new THREE.Vector3(-7,5.2,0).applyQuaternion(root.quaternion).add(root.position);look=equipmentLook(root);}camera.position.lerp(desired,1-Math.pow(.002,dt));camera.lookAt(look);}
  function update(dt){if(mode==='craneA'&&!viewAdjust){operateCrane(active,dt);updateAction();followCamera(dt);}else{cranePhysics.update(craneInput.read({x:0},new Set()),dt);if(mode!=='observe'&&!viewAdjust){if(mode.startsWith('crane'))operateCrane(active,dt);else if(mode==='excavator')operateExcavator(active,dt);else drive(active,dt);updateAction();followCamera(dt);}}}
  ...
  function resetMission(){stop();if(task===0){const rig=cranes[0];if(cranePhysics.attached)cranePhysics.disconnect();rig.rotor.rotation.y=rig.angle;rig.trolleyX=5.8;rig.trolley.position.x=5.8;rig.cableLength=4.7;cranePhysics.slewVelocity=cranePhysics.trolleyVelocity=cranePhysics.hoistVelocity=0;updateCraneCable(rig);rig.root.updateMatrixWorld(true);const p=new THREE.Vector3();rig.block.getWorldPosition(p);cranePhysics.debugPlaceCargo([p.x,.92,p.z]);rebarBeacon.visible=true;targetBeacon.visible=true;}else if(task===1){if(forklift.userData.load){scene.attach(forklift.userData.load);forklift.userData.load=null;}forklift.position.set(-9,.62,4.9);forklift.rotation.y=0;forklift.userData.forkHeight=.2;forklift.userData.fork.position.y=.2;palletCargo.position.set(-10.5,.86,4.9);palletBeacon.visible=true;}else{truck.position.set(-4,.62,6.5);truck.rotation.y=0;}updateAction();snapCamera();toast('当前任务已重置');}
  function prepareCranePickup(){if(!mode.startsWith('crane'))enter('craneA');active.cableLength=active.height-1;updateCraneCable(active);active.root.updateMatrixWorld(true);cranePhysics.debugPlaceCargoAtHook();updateAction();}
  function snapshot(){const crane=cranes[0],v=mode==='excavator'?null:active,cargoWorld=new THREE.Vector3();rebarCargo.getWorldPosition(cargoWorld);return{mode,selectedId,cameraMode,viewAdjust,inputCount:input.size+(joystick.pointerId===null?0:1)+excavPointers.size,joystick:{x:joystick.x,y:joystick.y},task,crane:{rotation:crane.rotor.rotation.y,hookHeight:crane.height-crane.cableLength,trolley:crane.trolleyX,cargoAttached:cranePhysics.attached,cargoWorld:cargoWorld.toArray(),...cranePhysics.snapshot()},excavator:{boom:excavator.userData.rig.boom.rotation.z,stick:excavator.userData.rig.stick.rotation.z,bucket:excavator.userData.rig.bucket.rotation.z,...excavatorPhysics.snapshot()},vehicle:v?.root?{position:v.root.position.toArray(),rotation:v.root.rotation.y,wheelRotation:v.wheelRotation,forkHeight:v.root.userData.forkHeight||null,loaded:!!v.root.userData.load}:null};}
  ...
  function setHydraulic(e,base){const rect=base.getBoundingClientRect(),max=rect.width*.3,dx=e.clientX-(rect.left+rect.width/2),dy=e.clientY-(rect.top+rect.height/2),length=Math.hypot(dx,dy)||1,scale=Math.min(1,max/length),x=dx*scale,y=dy*scale,side=base.dataset.hydraulic;excavInput[side+'X']=x/max;excavInput[side+'Y']=y/max;base.querySelector('i').style.transform=`translate3d(${x}px,${y}px,0)`;}
  excavatorControls.querySelectorAll('.hydraulic-base').forEach(base=>{base.addEventListener('pointerdown',e=>{e.preventDefault();if(excavPointers.has(e.pointerId))return;excavPointers.set(e.pointerId,{type:'hydraulic',base});base.classList.add('active');setHydraulic(e,base);buzz();},{passive:false});base.addEventListener('pointermove',e=>{if(excavPointers.get(e.pointerId)?.base===base)setHydraulic(e,base);},{passive:false});});
  excavatorControls.querySelectorAll('.track button').forEach(button=>button.addEventListener('pointerdown',e=>{e.preventDefault();const track=button.parentElement.dataset.track,key=track+'Track',value=Number(button.dataset.trackValue);excavPointers.set(e.pointerId,{type:'track',key,button});excavInput[key]=value;button.classList.add('active');buzz();},{passive:false}));
  function releaseExcavatorPointer(id){const p=excavPointers.get(id);if(!p)return;excavPointers.delete(id);if(p.type==='track'){excavInput[p.key]=0;p.button.classList.remove('active');}else{const side=p.base.dataset.hydraulic;excavInput[side+'X']=excavInput[side+'Y']=0;p.base.classList.remove('active');p.base.querySelector('i').style.transform='translate3d(0,0,0)';}}
  ...
  cargoButton.addEventListener('click',()=>{cargoAction();buzz(25);});cameraButton.addEventListener('click',toggleCamera);adjustButton.addEventListener('click',toggleAdjust);operateButton.addEventListener('click',()=>enter());document.querySelector('#exit-control').addEventListener('click',exit);document.querySelector('#task-action').addEventListener('click',focusMission);document.querySelector('#mission-reset').addEventListener('click',resetMission);document.querySelector('#control-reset').addEventListener('click',resetMission);window.addEventListener('pointermove',e=>{if(e.pointerId===joystick.pointerId)updateJoystick(e);const p=excavPointers.get(e.pointerId);if(p?.type==='hydraulic')setHydraulic(e,p.base);},{passive:false});window.addEventListener('pointerup',e=>{releasePointer(e.pointerId);releaseExcavatorPointer(e.pointerId);if(e.pointerId===joystick.pointerId)resetJoystick();},true);window.addEventListener('pointercancel',stop,true);window.addEventListener('touchcancel',stop,{passive:true,capture:true});window.addEventListener('blur',stop);window.addEventListener('orientationchange',stop);document.addEventListener('visibilitychange',()=>{if(document.hidden)stop();});
...
window.siteDiagnostics={renderer,scene,camera,controls,selectable,revision:THREE.REVISION,selectObject,clearSelection,simulator,cranes,vehicles,physicsReady:true,physicsWorld,cranePhysics,excavatorPhysics};
